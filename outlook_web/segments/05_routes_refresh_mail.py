...
def fetch_account_folder_emails(account: Dict[str, Any], folder: str, skip: int, top: int,
    ...
    def try_imap() -> Optional[Dict[str, Any]]:
        if explicit_folder_id:
            all_errors['imap'] = 'folder_id 参数仅走 Graph 路径'
            return None
        for server, label in ((IMAP_SERVER_NEW, 'IMAP (New)'), (IMAP_SERVER_OLD, 'IMAP (Old)')):
            ...
            if imap_result.get('success'):
                ...
                emails = format_email_items(imap_result.get('emails', []), storage_folder)
                for item in emails:
                    item['folder'] = storage_folder
                    if explicit_mailbox:
                        item['mailbox'] = explicit_mailbox
...
def fetch_email_detail_for_account(account, message_id, method='graph', folder='inbox',
    ...
    if stored_channel:
        ...
        channel_order = ('imap',)
    elif is_microsoft_mailbox_account(account):
        channel_order = ('graph', 'imap')
    else:
        channel_order = ('imap', 'graph')
    last_method_label = ''
