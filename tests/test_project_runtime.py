...
class FrontendEmailListSecurityTests(unittest.TestCase):
    ...
    def test_email_active_highlight_uses_selection_key(self):
        ...
        self.assertIn('findEmailForListAction(messageId, index)', select_source)
        self.assertIn('requestMarkEmailsAsRead([', select_source)
        self.assertIn('function findEmailForListAction(messageId, index)', self.emails_js)
        self.assertIn('applyToEmailList(statusFilterOverrideEmails)', self.emails_js)
        self.assertNotIn('applyEmailReadState(normalizedItems, !targetIsRead)', self.emails_js)
    ...
    def test_unread_filter_accepts_numeric_and_camelcase_read_state(self):
        ...
        self.assertIn('list = list.filter(email => isEmailUnread(email));', self.emails_js)
        self.assertIn('list = list.filter(email => isEmailFlagged(email));', self.emails_js)
        self.assertIn('list = list.filter(email => emailMatchesKeyword(email, keyword));', self.emails_js)
    ...
    def test_email_list_has_fetch_all_and_keyword_search(self):
        ...
        self.assertIn('function stripEmailSearchText(value)', self.emails_js)
        self.assertIn('email?.body', self.emails_js)
        self.assertIn('email?.keyword_hit === true', self.emails_js)
